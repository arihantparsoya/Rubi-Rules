...
(* Mathematicas built-in PolynomialQ, Exponent and Coefficient functions can return erroneous results because they do not *)
...
FixInertTrigFunction[u_.*(c_.*sec[v_]^n_.)^p_.*w_,x_] :=
  (c*cos[v]^(-n))^p*FixInertTrigFunction[u*w,x] /;
FreeQ[{c,p},x] && PowerOfInertTrigSumQ[w,cos,x] && IntegerQ[n] && IntegerQ[p]
